...
async def start(message: types.Message) -> None:
    await message.reply("Привет)", reply_markup=commands_keyboard)#текст нужно придумать
...
async def postgraduate(call: types.CallbackQuery) -> None:
    await call.message.answer("ПОИСК ИНФОРМАЦИИ НАЧАЛСЯ")
    url = "https://apply.innopolis.university/postgraduate-study/?lang=ru&id=12&site=s1&template=university24&landing_mode=edit"
...
async def trash(message: types.Message) -> None:
    await message.answer("Вы ввели неправильную комманду.\nУзнать условия поступления:", reply_markup=commands_keyboard)


def register_handlers(dp: Dispatcher) -> None:
    dp.register_message_handler(start, commands=["start", "help"])
    dp.register_message_handler(trash)
    dp.register_callback_query_handler(bachelor, lambda c: c.data == "bachelor")
    dp.register_callback_query_handler(master, lambda c: c.data == "master")
    dp.register_callback_query_handler(postgraduate, lambda c: c.data == "postgraduate")
